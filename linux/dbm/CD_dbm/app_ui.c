...
static menu_options show_menu(const cdc_entry  *current_cdd);
...
static void announce(void){
        printf("\n\nWelcome to the demonstration CD catalog database programe\n");
}
        ...
        while(option_chosen == mo_invalid){
                if(cdc_selected->catalog[0]){
                        ...
                        switch(tmp_str[0]){
                                ...
                                case '3': option_chosen = mo_count_entryies;break;
...
static int enter_new_cat_entry(cdc_entry *entry_to_update){
        ...
        strcpy(new_entry.catalog,tmp_str);

        printf("Enter title: ");
        (void)fgets(tmp_str,TMP_STRING_LEN,stdin);
        strip_return(tmp_str);
        strcpy(new_entry.title,tmp_str);

        printf("Enter type: ");
        (void)fgets(tmp_str,TMP_STRING_LEN,stdin);
        strip_return(tmp_str);
        strcpy(new_entry.type,tmp_str);

        printf("Enter artist: ");
        (void)fgets(tmp_str,TMP_STRING_LEN,stdin);
        strip_return(tmp_str);
        strcpy(new_entry.artist,tmp_str);

        printf("\n New catalog entry entry is: \n");
        display_cdc(&new_entry);
        if(get_confirm("Add the Entry?")){
...
static void enter_new_track_entries(const cdc_entry *entry_to_add_to){
        ...
        printf("Press return to leave exsisting description unchanged,\n");
        printf(" a single to delete this and remaning tracks,\n");
...
static void display_cdt(const cdt_entry *cdt_to_show){
        printf("catalog: %s ,track_no: %d ,track_txt: %s\n",cdt_to_show->catalog,cdt_to_show->track_no,cdt_to_show->track_txt);
}
