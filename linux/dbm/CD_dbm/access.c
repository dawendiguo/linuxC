...
int database_initialize(const int new_database){
        ...
        cdc_dbm_ptr = dbm_open(CDC_FILE_BASE,open_mode,0640);
        cdt_dbm_ptr = dbm_open(CDT_FILE_BASE,open_mode,0640);
...
cdc_entry get_cdc_entry(const char *cd_catalog_ptr){
        ...
        memset(&entry_to_find,'\0',strlen(entry_to_find));
        strcpy(entry_to_find,cd_catalog_ptr);

        local_key_datum.dptr = (void *)entry_to_find;
        local_key_datum.dsize = strlen(entry_to_find);
        ...
        if(local_data_datum.dptr){
                memcpy(&entry_to_return,(char *)local_data_datum.dptr,local_key_datum.dsize);
        }
        return(entry_to_return);
}  /* get_cdc_entry */


cdt_entry get_cdt_entry(const char *cd_catalog_ptr,int track_no){
        ...
        /* set up the search key, which is a composite key of catalog entry and track number */
        memset(&entry_to_find,'\0',strlen(entry_to_find));
...
int del_cdc_entry(const char *cd_catalog_ptr){
        ...
        memset(&key_to_del,'\0',strlen(key_to_del));
...
cdc_entry search_cdc_entry(const char *cd_catalog_ptr,int *first_call_ptr){
        ...
        /* check ptr and input valiable */
        if( !cdc_dbm_ptr || !cdt_dbm_ptr) return(entry_to_return);
        if(!cd_catalog_ptr || !first_call_ptr) return(entry_to_return);
        if( strlen(cd_catalog_ptr) >= CAT_CAT_LEN) return(entry_to_return);

        /* protect against never passing *first_call_ptr true , 表示从文件起始搜索*/
        if(local_first_call){
                local_first_call = 0;
                *first_call_ptr = 1;
        }
