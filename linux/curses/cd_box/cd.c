...
int getchoice(char *greet, char *choice[]){
        ...
        while( key != 'q' && key != KEY_ENTER && key != '\n'){
                ...
                draw_menu(choice,selected_row,start_screenrow,start_screencol);
...
void update_cd(){
        ...
        clear_all_screen();
        mvprintw(PROMPT_LINE,0,"Re_entering tracks for CD. ");
        if(!get_confirm())
                return;
        move(PROMPT_LINE,0);
        clrtoeol();
...
void find_cd(){
        ...
        mvprintw(Q_LINE,0,"Enter a string to search for in CD title: ");
        get_string(match);
...
void list_tracks(){
        ...
        /* First count the number of tracks for the currennt ID */
        tracks_fp = fopen(tracks_file,"r");
        if(!tracks_fp)
                return;
        while(fgets(entry,MAX_ENTRY,tracks_fp)){
        ...
        /* Make a new pad, ensure that even if there is only a single track the PAD is large enough so the later prefresh() is always valid. */
